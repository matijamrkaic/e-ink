...
```bash
pip3 install -r requirements.txt   # garminconnect + Pillow
pip3 install requests              # only needed for weather.py
```
...
2. **Generate auth tokens locally** (datacenter logins trip Garmin's MFA, so CI
...
re-run step 2 and update the `GARMINTOKENS` secret.
